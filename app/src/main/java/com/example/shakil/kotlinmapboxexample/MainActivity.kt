...
class MainActivity : AppCompatActivity(), OnMapReadyCallback, PermissionsListener, MapboxMap.OnMapClickListener {
    ...
    //funcoes criadas para dar o start no mapa
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (resultCode == RESULT_OK && requestCode == REQUEST_CODE_AUTOCOMPLETE) {
            ...
            /** Crie uma nova FeatureCollection e adicione uma nova Feature a ela usando selectedCarmenFeature acima.
             * Em seguida, recupere e atualize a fonte designada para mostrar o ícone da camada de símbolo de um local selecionado **/
            if (mapboxMap != null) {
                ...
                if (style != null) {
                    ...
                    /** Mova a câmera do mapa para o local selecionado **/
    ...
    override fun onMapClick(point: LatLng): Boolean {
        ...
        //Criação do evento da mensagem que me retorna as coordenadas clicadas pelo usuário no mapa

        val latLng = LatLng(point.latitude, point.longitude)
        Toast.makeText(this@MainActivity, "Atualização de localização : $latLng", Toast.LENGTH_SHORT).show()
                ...
                .getRoute(object : Callback<DirectionsResponse> {
                    override fun onResponse(call: Call<DirectionsResponse>, response: Response<DirectionsResponse>) {
                        ...
                        if (response.body() == null) {
                            Log.d(TAG, "No routes found, mak e sure you set the right user and access token")
                            return
                        }
    ...
    private fun enableLocationComponent(loadedMapStyle: Style?) {
        // Verifique se as permissões estão habilitadas e, se não, solicite
        if (PermissionsManager.areLocationPermissionsGranted(this)) {
            // Ative o MapboxMap LocationComponent para mostrar a localização do usuário
            // Adicionar em LocationComponentOptions também é um parâmetro opcional
